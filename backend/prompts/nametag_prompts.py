...
def get_logo_image_prompt(brand_name: str, direction_text: str) -> str:
    return f"""당신은 세계적인 수준의 브랜드 아이덴티티(BI) 전문 디자이너입니다.
...
[엄격한 품질 및 스타일 요구사항]
- 스타일: 사용자가 인터뷰에서 선택한 스타일 고수 해야함. 만약 인터뷰에 스타일이 없다면 최대한 회사의 분위기에 맞게 스타일을 선정 할 것
- 로고에는 우리 브랜드를 나타내는 심볼이나 엠블럼이 꼭 들어가야 함
- 그 아래에 회사명 글자를 넣어야 함.
- 심볼이나 엠블럼이 글자보다 무조건 커야하고 엠블럼이나 심볼이 중요할 경우 글자를 꼭 넣지 않아도 됨.
- 배경: 완벽한 순백색(Solid White, #FFFFFF) 배경.
- 디테일: 복잡한 스케치나 번짐 효과 없이 선명하고 또렷한 가장자리 유지.
- 텍스트 제어: '{brand_name}' 외에 의미를 알 수 없는 AI 문자나 기호가 절대 포함되지 않도록 할 것.
